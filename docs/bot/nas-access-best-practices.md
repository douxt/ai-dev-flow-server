...
# 查看容器状态
ssh root@nas "$DOCKER ps"

# 容器内执行 Python
ssh root@nas "$DOCKER exec langbot /app/.venv/bin/python3 ..."
ssh root@nas "$DOCKER exec langbot-plugin /app/.venv/bin/python3 ..."

# 重启（顺序重要！）
ssh root@nas "$DOCKER restart langbot-plugin langbot"

# 传文件进容器
scp file.py root@nas:/tmp/ && ssh root@nas "$DOCKER cp /tmp/file.py langbot-plugin:/path/"

# 读插件日志
ssh root@nas "$DOCKER exec langbot-plugin cat /tmp/silent_init.log"
ssh root@nas "$DOCKER exec langbot-plugin cat /tmp/silent_gate.log"
```
...
ssh -nT -o BatchMode=yes -o ConnectTimeout=6 root@nas \
  'docker exec langbot base64 -w0 /app/data/langbot.db' > /tmp/x.b64
...
`ssh ... docker exec ... | 管道` 连续调用会在 NAS 上堆积僵死的 exec 会话，后续命令全部 hang。

**解法**：合并成**单次 exec 跑完多条命令**，或重启容器清僵尸：
```bash
# 一条命令做完所有事，不连击
ssh root@nas '/usr/local/bin/docker exec langbot-plugin sh -c "
  echo ===A===; grep -c pattern /tmp/silent_gate.log
  echo ===B===; grep -F keyword /tmp/silent_gate.log | tail -10
"'
```
