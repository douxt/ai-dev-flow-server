...
- ~~B1：GENERATE 单轮真伪裁决闸门~~（2026-09-02 二次调研证伪）→ write-time 内容筛选拦截 0/360 投毒记忆（arXiv 2608.21230：判伪需文本外 grounding，1.2% 污染打掉 2/3 效用）；单轮只见"纠正句+上条回复"结构性判不了，裁决须看完整事件弧——由 B 线批量化承接（§三-A）
- ~~限流四层配额位置修补~~（default.py:898 检测前吃配额、无关消息挡真纠正，9/02 测试实锤）→ 不打补丁，批量化后事件驱动天然限流，整层拆除留日 cap 兜底（§三-A B 线）
...
**前车之鉴登记**：Mem0 v3 砍掉自动冲突消解改 ADD-only → 陈旧矛盾冒头（issue #4956）——本方案下游 merge/confirm 消解链**保留不动**，批量层只加入口关。**Q7 触发式 critic（§三-B 中期项）复用 consolidator 骨架**，不另起炉灶。

> 上方 P1 交互流程示例中"gate: 话语重写→检测→即时生成"实时链为本线替代对象；行为防线（Q1+链尾证据校验，§三-B）不依赖 lesson 即时入库，分钟级学习时延可接受。计划详情待实施：批准稿见会话计划（reflection-consolidator）。
...
> **状态 2026-09-02：Q1-Q5 ✅ 已上线**（V4-V6 正反行为测试全过，含谄媚旧反思近距离在场仍守住的最强验证；执行结果与判据修正见 docs/plans/2026-09-02-chat-quality-prompt.md；事实源 docker/langbot/prompts/system-prompt-v3.md）。V8 副作用观察窗至 ~9/09，两周后（~9/16）评估 Q6/Q7 是否推进。
...
- **Q7 触发式 critic**（挂点更新 9/02：复用 §三-A B 线 consolidator 骨架，不另起炉灶）：仅判断句（为什么/是不是/该不该）回复后异步独立调用自检——外部信号绕开 self-critique 失效条件；发现硬伤走"补充更正"通道，复用 merge/confirm 机制沉淀（与 §三-A P2 合流）。对应四级进化**第二级（自我评估层）**的具体落地形态。
...
1. ~~**Q1-Q5（1 天）**~~ ✅ 2026-09-02 已上线（见 §三-B 短期包状态注）：反谄媚/前提审查/弃权/人设负约束/arena 护栏——一次 pipeline prompt 改动全含，改口率+8/24 素材重放做验收
