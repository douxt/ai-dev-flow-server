...
### 收官确认（2026-09-11，用户反馈）

- **告警通道确认可用**：用户确认 Telegram 收到消息（4 条：T1 测试、restart 告警、看门狗自检、漂移演练）
- **T9 关闭**：不配置密码回退（用户判断 2026-09-10 的掉线是**风控踢下线**，密码登录同样可能被拦）→ 人工扫码 + `qq-offline` 告警是唯一防线；T2 的"约 7 天失效"假设降级为未确认（风控场景下时间不可预测）
- **看门狗调度**：脚本在开发机（需要仓库 main 做漂移比对），cron 需用户在开发机执行一行；若开发机非 24/7，可选拆分（NAS 存活/心跳检查移到 24/7 的云服务器，漂移比对留开发机）——待用户确认

### 架构修正（2026-09-11，用户反馈：开发机常关机、NAS 常开）

检测搬到 NAS（24/7），发送留在阿里云（唯一 Telegram 通道），开发机看门狗降级为可选：

```
NAS  health-check (*/5) ─┐
NAS  selfcheck   (*/15) ─┼─> state/alert ──云 cron */5 拉取──> Telegram
NAS  deep-smoke  (17 */6)┘                        └─ 每日 09:00 摘要（死者开关）
```

- `nas/selfcheck.sh`：巡检是否停跳（心跳年龄）+ 在版脚本是否被改动（对照 `state/expected.md5` 8 项）+ 待发告警积压
