...
## 决策与关闭（2026-09-11，用户决定）

**选 C：不配置密码回退，保持"告警 + 人工扫码"。**

用户判断：本次（2026-09-10 17:56）的会话失效**应是风控踢下线**，配置密码回退的实际收益有限（风控场景下密码登录同样会被拦截），因此不做凭据落盘。

- [x] `[decision]` AC1: 选择 **C**，理由=用户判断为风控踢下线
- [x] `[human-verify]` AC4: 文档写明扫码入口与预期（`docs/bot/nas-access-best-practices.md` §十二：`http://nas:6099`，token `udimc123`；巡检只报 `ACCOUNT-OFFLINE`/`qq-offline`，不自动恢复）
- [ ] AC2/AC3（配置落地与演练）：**不做**

### 连带影响（已在文档中体现）

- T2 的"约 7 天会话失效"假设**降级为未确认假设**：若为风控，则复发时间不可预测 → 告警（T1 的 `qq-offline`）成为唯一防线
- 复发时的处置脚本化路径见 `skills/nas-ops` §5（扫码 → 验证 `get_login_info`/`link=1`/插件事件 → 取证）
