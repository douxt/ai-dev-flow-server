...
| S4 | 异常路径 4 类覆盖 | #4 |
| S5 | 外部依赖接口签名明确（含错误码枚举、权限检查机制） | #5 |
| S6 | 工作量估算 ≤5d | #7 |
| S7 | Ponytail 四问已写入 §Decisions | P1-P4 |
| S8 | 三假设（技术/行为/边界）已审计 | H1-H3 |
| S9 | 验证方案标注等级（L1-L5） | 验证层级 |
| S10 | spec.md 存在且非空 | 产物检查 |
| S11 | Testing 段完整 — 按 test-plan-template 填写：接缝选择 + 模块策略 + Stub决策 + 场景清单（含可执行性标注）+ 冒烟验证 + AC覆盖度矩阵 | 测试宪法 |
| S12 | 遗留代码特征测试 — 若涉及零测试覆盖模块，spec 中标注特征测试需求，/to-tickets 据此拆出前置 `[legacy]` ticket | 特征测试宪法 |
| S13 | 测试层级分配合理 — E2E 占比 ≤ 15%（must-pass），每项 E2E 标注决策路径理由（`判断2→跨系统流程: <系统名>`）；[hotfix] ticket 自动豁免、[no-test] 标注后豁免 | 分层宪法 |

## 通过条件

S1-S5 + S10-S13 必须通过。S6-S9 为 advisory 警告。
...
| 中型 | spec 50-200 行 / 1-2 个模块 | 自查本清单 S1-S13，逐项确认 |
