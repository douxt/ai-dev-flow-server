...
| C0.8 | 断言强度分布（全局+单文件+操作行级） | ①全局：弱断言占比 > 50% → FAIL=1。②单文件：有操作但零强断言 → FAIL=1。③操作行级：操作后 5 行内仅弱断言 → FAIL=1 | ❌ 硬阻断（v2.8 升级）——`expect(table).toBeVisible()` 作为操作后唯一断言不区分成败。规则：click/fill/submit 之后至少有 1 条验证操作结果的强断言（ASI 3-5 级），不能只有"元素仍可见"。参考 [ASI 五级量表](../knowledge/12-断言强度指数.md)|
...
[CX] 断言最终形式: /tdd 内以最终业务行为写入，GREEN 阶段未改测试 — ✅
